% Figures 6-9 – Average Albedos for all models at INDIVIDUAL AWSs 2010-2021
% by Maxim Altan-Lu Shapovalov for a thesis manuscript
% Corrected Oct 25 2024

% Specifically, this code is made to create the box and rmse plots for
% individual stations; previously, only the average was considered, now, every
% year will be accounted for. 
...
% get the years for the specific station from the function
yrz = specifyYears(aws{1});
...
for a = 1:length(yrz) % 1st, loop thru weather stations
    met = aws{1}; % choose current aws
    % prep for storage vars for albedo and melt
    alb = NaN(8760,length(data)); m50 = alb;
    ...
    % extract yearly alb and melt data
    yrr = yrz{a}; % get year which is analyzed
    % Get yearly albedo data
    for b = 1:length(data)
        if b == 1% if modis (mcd43a3)
            load([mp 'input/userdata/mcd43a3/single pixel/' data{b} '_' met...
                '_' yrr '.mat']) % load "Data"
        else % if other models
            load([mp 'input/userdata/' data{b} '_' met '_' yrr '.mat']) % load "Data"
        end
        alb(:,b) = Data.albedo; % save individually
    end

    % Get yearly melt data
    for b = 1:length(data)
        if b==1 % if modis (mcd43a3)
            load([mpn 'output/mcd43a3/single pixel/ice1/' met '_' data{b} '_' yrr '.mat'])
        else
            load([mpn 'output/ice1/' met '_' data{b} '_' yrr '.mat'])
        end
        m50(:,b) = ice1.melt; % save individually
    end

    % rename data for code consistency (i.e., chunks of code below were written
    % with vars avgsdata, avg50, and tiempo1 in mind)
    avgdata = alb; avg50 = m50; tiempo1 = Data.Time;
...
switch baropt
    case 'alb'
        ...
        y4 = ra4;
        y5 = ra5;
        rect_height = 0.18; % top bound for rectangle
...
%choose location of legend on the bottom
lgdloc = 'left';
% lgdloc = 'right';
...
switch baropt
    case 'alb'
        ...
        y4 = col4;
        y5 = col5;
        rh_t = 0.4; % top bound for rectangle
    ...
    case 'melt'
        ...
        y4 = col9;
        y5 = col10;
        rh_t = 0.3; % top bound for rectangle
...
%         rect_height = 0.6; % top bound for rectangle
        rh_t = 220; % top bound for rectangle
        rh_b = -60; % bottom bound for rectangle
...
switch lgdloc
    case 'left'
        lgdd = [0.158, 0.151, 0.1, 0.11]; % bottom left, in P1 lightest grey box
    case 'right'
        lgdd = [0.775, 0.151, 0.1, 0.11]; % bottom right, in P5 lightest grey box
end
% Add labels and title
switch baropt
    case 'alb'
...
%         title("Bias of each model's albedo relative to MODIS albedo");
        legend('MERRA-2', 'MAR', 'RACMO','Snow','Snowline','Ice','Position',lgdd)
    case 'melt'
        ylabel('Melt [m]');
